...
| `kinematics` | `v_px_per_s`, `direction_deg`, `uniform` (false: do not quote `v_px_per_s`), `fit`, `omega_rad_per_s`, `relative_speed_m_per_s`, `lower_bound_m_per_s`, `body_lengths_per_s` (the body's own length only if the object is resolved: for a point, `--size-px` is its blur), `scale_bar_m_per_s`, `missing` |
| `comotion` | `verdict`, `finding`, `pairs`, `whole.{rel_D, rel_D_per_s, obj_D, flow_D, dt, …}`, `legs`, `sweep`, `flow_near_zero_zone` |
| `symbology` | `boresight.{x, y, how}`, `method`, `trial` (a method chosen by `auto` is tried on 20 frames first: `{frames, solved}`, and none solved ends it there), `frames_solved`, `radius`, `radius_is_fixed` (false: the angles are unreliable), `position_step_px` and `theta_step_deg` (the template places the glyph to whole pixels, hue to half; chroma finer: null), `theta_deg_per_px_of_boresight` (theta is only as good as the boresight), `rotation[]` each with `dtheta_dt`, `resolvable_deg_per_s` (slower than one step over the window is not seen) and `sense`, `corner_brackets` |
